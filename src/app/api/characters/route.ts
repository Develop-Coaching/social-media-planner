...
export async function GET(request: NextRequest) {
  try {
    ...
    const characters = await getCharacters(effectiveUserId, companyId);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { effectiveUserId } = await resolveCompanyAccess(userId, role, companyId);
    const character = await addCharacter(effectiveUserId, companyId, name.trim(), description?.trim() || "");
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (image !== undefined) {
      if (image === "") {
        await deleteCharacterImage(effectiveUserId, companyId, characterId);
      } else {
        // Validate size (max ~5MB base64)
        if (image.length > 7_000_000) {
          return NextResponse.json({ error: "Image too large (max 5MB)" }, { status: 400 });
        }
        await uploadCharacterImage(effectiveUserId, companyId, characterId, image);
      }
    }
    ...
    if (Object.keys(textUpdates).length > 0) {
      const updated = await updateCharacter(effectiveUserId, companyId, characterId, textUpdates);
      if (!updated) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { effectiveUserId } = await resolveCompanyAccess(userId, role, companyId);
    const removed = await deleteCharacter(effectiveUserId, companyId, characterId);
    if (!removed) {
      return NextResponse.json({ error: "Character not found" }, { status: 404 });
    }
